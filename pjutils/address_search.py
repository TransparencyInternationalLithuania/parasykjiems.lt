...
def search_for_addr(records, value, field):
    if field=='city':
...
def get_addr_suggests(table, entered):
    suggestions = []
    ...
    if len(entered_list)>1:
        ...
        if len(entered_list)>2:
            ...
            district = ''
        records = search_for_addr(table.objects.all(), district, 'district')
        records = search_for_addr(records, city, 'city')
        records = search_for_addr(records, street, 'street')
    else:
        street_list = entered_list[0].split(' ')
        street_list.pop()
        street = ' '.join(street_list)
        records = search_for_addr(table.objects.all(), street, 'street')
